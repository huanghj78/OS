...
/* get sidin_buf from input terminal
   ...
   select connect_sn by the descritor @**** in start of send_buf */
    ...
    //modify2
    int fp = open("./log.txt",O_RDWR|O_CREAT|O_TRUNC,0666);
    if(fp == -1){
        ERR_EXIT("./log.txt fopen()");
    }
    //
    ...
    while(1) { 
        ...
        while (1) { /* update sn_stat and nickname from recv_send_data(), or brocast msg to all sn */
                ...
                if(msg_buf[5] == '1') { /* #1name: renaming the nickname */
                    ...
                    if(i > max_sn) {
                        ...
                        if(stat == STAT_ACCEPTED) {
                            ...
                            char* f;
                            f = fgets(tmp,MSG_SIZE,fp);
                            while(f!=NULL){
                                flags = fcntl(fd[sn][1], F_GETFL, 0);
                                fcntl(fd[sn][1], F_SETFL, flags | O_NONBLOCK);
                                ret = write(fd[sn][1], tmp, MSG_SIZE); /* non-blocking write ord-pipe */
                                if(ret <= 0) {
                                    printf("sn = %d write error, message missed ...\n", sn);
                                }
                                f = fgets(tmp,MSG_SIZE,fp);
		    ...
		    sprintf(msg_buf, "========\n");
                ...
                memset(msg_buf, 0, MSG_SIZE);
                //modify1
                sprintf(msg_buf, "You@: %s", stdin_buf);
                 ret = write(fd[sn][1], msg_buf, MSG_SIZE); /* non-blocking write ord-pipe */
                 if(ret <= 0) {
                    printf("sn = %d write error, message missed ...\n", sn);
                }
                ...
                char tmp[MSG_SIZE];
                sprintf(tmp,"%s\n",msg_buf);
                int fw = write(fp,tmp,strlen(tmp));
                if(fw == -1){
                    ERR_EXIT("fwrite()");
                 }
                 //

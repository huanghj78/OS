...
int main() {
  int thr_num, task_capacity, task_num, i,ret;
  ...
  if (pool == NULL) {
    ...
    ret = sem_init(&(pool->empty_slots), 1, task_capacity); /* the second parameter of sem_init must be set to non-zero for inter process sharing */
    if (ret == -1) {
        perror("sem_init-mutex");
    }

    ret = sem_init(&(pool->count_mutex), 1, 1); /* the second parameter of sem_init must be set to non-zero for inter process sharing */
    if (ret == -1) {
        perror("sem_init-mutex");
    }

    ret = sem_init(&(pool->thread_mutex), 1, 1); /* the second parameter of sem_init must be set to non-zero for inter process sharing */
    if (ret == -1) {
        perror("sem_init-mutex");
    }
